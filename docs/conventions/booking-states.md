...
| --- | --- | --- | --- | --- |
| **Approved cancellation** (W4: khách xin hủy booking PAID, admin duyệt) | `CANCELLED` + `cancelledAt` | 0..1 row — số tiền do BẬC CHÍNH SÁCH quyết (ADR-0030); **0 row** khi sổ đã settle từ trước (ADR-0029 §2) | **Trả lại** (`seats_booked -= party`) — BẤT KỂ hoàn bao nhiêu | Khách chủ động thôi đi tour — chuyến đi kết thúc. Tiền đã trả nằm trọn trong ledger; status kể chuyện ghế/chuyến đi, không kể chuyện tiền. CancellationRequest → `REFUNDED` (enum của model — "resolved by refund"). |
