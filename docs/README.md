...
| P3b — nối API bước 1: cụm Blog + nền `lib/api` | [2026-07-31-blog-api-design](specs/2026-07-31-blog-api-design.md) | ✅ spec duyệt 31/07 — seed 9 bài từ mock đã duyệt (sections → markdown), nền `lib/api` theo [ADR-0016](adr/0016-web-data-layer.md), đổi nguồn `/blog` + `/blog/[slug]` + rss + sitemap + teaser Home (điểm mới cần xác nhận), bảng shape-gap mock ↔ contract |
...
| [Bước 1 nối API: cụm Blog + nền lib/api](plans/2026-07-31-blog-api.md) | 10 task theo [spec 31/07](specs/2026-07-31-blog-api-design.md): deps+env → client+tags → tri-state → seed 9 bài (⚠️ bảng dời 4 ngày mock ở tương lai về quá khứ — ADR-0004 lọc `publishedAt <= now`) → VM+fetch → markdown/TOC → 2 trang + rss + sitemap + teaser Home → khai tử mock journal + nghiệm thu | 🚧 chờ thi công |
