...
| P3b — nối API bước 4: cụm Destinations + xoá lệch mock | [2026-07-31-destinations-api-design](specs/2026-07-31-destinations-api-design.md) | ✅ spec duyệt 31/07 — trọn gói lệch-nhìn-thấy (2 trang destinations + Home + /about + navbar 19 điểm + moments sửa slug); sau branch này `mocks/tours.ts`/`mocks/destinations.ts` chết hẳn; rủi ro lớn nhất: layout fetch (navbar) chạm mọi route |
...
| [Bước 4: cụm Destinations lên API + xoá lệch mock](plans/2026-07-31-destinations-api.md) | 7 task theo [spec 31/07](specs/2026-07-31-destinations-api-design.md): lib/regions sang VM → /destinations → 3 trang vùng (reviews compose per-tour) → Home + moments hết link chết → /about → navbar 19 điểm (⚠️ layout fetch — có điều khoản DỪNG) → khai tử mocks/tours + destinations + nghiệm thu "0 link tour chết toàn site" | 🚧 chờ thi công |
