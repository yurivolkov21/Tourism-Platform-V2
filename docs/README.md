...
| P3b Web | [ADR-0011](adr/0011-p3b-web-architecture.md) | 🚧 Home ✅ + navbar ✅ + /about ✅ + /contact ✅ + auth 6/6 ✅ + gia vị bản đồ ✅ + cụm pháp lý/utility ✅ 25/07 + **cụm Blog ✅ merge 27/07** (/blog · /blog/[slug] · rss.xml) + thân trang 404 & nền lưới động Contact ✅ 27/07 + **cụm Tours ✅ merge 27/07** (`/tours` · `/tours/[slug]` · robots.txt · sitemap.xml) + **hình ảnh & uy tín trang chi tiết ✅ merge 28/07** (card thiết kế lại 5 vòng · gallery khảm + lightbox · khu Traveller reviews); **cụm Destinations ✅ merge 30/07** (`/destinations` · `/destinations/[region]` ×3 · ADR-0015 rút tint vùng toàn site · chuyển động 3 trục miền); **chính sách ảnh hiện hành: toàn site dùng `ImagePlaceholder`, chỉ đổi ảnh thật khi user yêu cầu riêng**; **bước 1 nối API ✅ merge 31/07** (cụm Blog + nền `lib/api` — trang đầu tiên rời mock, SSG→ISR 300s + cache-tag); **kế tiếp**: bước 2 `/tours` (⚠️ phải quyết nội dung trước: 16 tour mock đã duyệt ↔ 23 tour seed API lệch slug gần như hoàn toàn), rồi bước 3–10 theo thứ tự docs-audit 30/07 |
...
| P3b — nối API bước 2+3: catalogue thật + `/tours` + detail | [2026-07-31-tours-catalogue-api-design](specs/2026-07-31-tours-catalogue-api-design.md) | ✅ spec duyệt 31/07 — seed lại TRỌN BỘ ~30 tour "như thật" (itinerary mốc giờ trong description, departures tương lai tĩnh tới 01/2027, reviews CURATED + 6 tour 0-review chủ đích), roster 30 dòng + tour mẫu Vũng Tàu 2N1Đ; nửa B gộp listing + detail (nối riêng listing = card trỏ 404) | 
...
| [Bước 2+3 nối API: catalogue thật + /tours + detail](plans/2026-07-31-tours-catalogue-api.md) | 10 task theo [spec 31/07](specs/2026-07-31-tours-catalogue-api-design.md): tách fixtures theo miền → content Bắc/Trung/Nam (mẫu Vũng Tàu đối chiếu từng dòng) → reviews CURATED + recompute rating → reset DB + đo API → lib/api/tours (VM = type contract) → listing (facet destinations PHẢI từ API, không thì filter chết với slug mới) → detail (itinerary `whitespace-pre-line`, gallery degrade media rỗng) → sitemap 52 URL + nghiệm thu 6 mục | 🚧 chờ thi công |
