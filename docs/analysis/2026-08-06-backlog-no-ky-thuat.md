...
| A8 | ~~**Ảnh tour vẫn rỗng**, không có lệnh nào lấp được~~ — **đường ống đã dựng xong 14/08** (nhánh `feat/media-inbox`, CHANGELOG cùng ngày): cây `media-inbox/` + 4 lệnh `media:tree`/`fetch`/`scan`/`upload`; user gửi link, máy khuân, duyệt mắt trước upload. **Nợ còn lại giờ là NỘI DUNG chứ không phải công cụ** (cập nhật cuối 14/08): 4/30 tour có cover, **9/19 địa danh có ảnh**, 5/19 địa danh có gallery, **15 khe site thì 9 đã có ảnh**. Gallery 7 thumb và nhánh HAI CỘT của `TourMediaPanel` vẫn **chưa render từ dữ liệu thật** vì chưa địa danh nào đủ ~7 ảnh | `/tours/[slug]` | 🚧 công cụ xong, còn thiếu ảnh |
