/**
 * Fixture 9 bài blog — port NGUYÊN VĂN từ mock đã duyệt của web
 * (`apps/web/src/mocks/journal.ts`, mảng `JOURNAL_POSTS`). Không đổi một chữ
 * nào trong title/excerpt/paragraph/bullet — đó là copy đã được duyệt.
 *
 * Convert mỗi bài theo 3 quy tắc:
 *   1. `sections[]` → một chuỗi markdown `content`: mỗi section là
 *      `## <heading>`, tiếp theo các đoạn văn (mỗi đoạn cách nhau một dòng
 *      trống) hoặc bullet dạng `- <bullet>`.
 *   2. `publishedAt`: mock có 9 ngày, 4/9 nằm ở TƯƠNG LAI so với hôm nay
 *      (2026-07-31). `publishedPostWhere()` (ADR-0004) lọc `publishedAt <=
 *      now`, nên giữ nguyên ngày mock sẽ khiến 4 bài biến mất khỏi `/blog`.
 *      Dời toàn bộ về quá khứ, giữ nguyên thứ tự tương đối, theo bảng cố định
 *      (khoá = ngày gốc trong mock, duy nhất từng bài):
 *
 ...
 *
 *   3. `tags`: tag chính = `category` mock (slugify thủ công); cộng một tag
 *      địa danh nếu bài xoay quanh rõ một nơi (đọc thấy tên nơi đó lặp lại
 *      trong section, không chỉ nhắc thoáng qua). Tên tag tiếng Anh (copy
 *      user-facing).
 *
 * KHÔNG có `authorId` — gán lúc runtime trong seed.ts bằng admin đã upsert.
 */
...
export const posts = [
  {
    ...
    content: [
      ...
      '',
      '## What actually earns its space in the bag',
      '',
  ...
  },
  {
    ...
    content: [
      '## Cao lầu, and why the noodles taste different here',
      '',
  ...
  },
  {
    ...
    content: [
      ...
      '',
      '## The lake and the poetry pavilion',
      '',
      ...
      '',
      '## Khải Định — concrete, mosaic, and a different century',
      '',
      ...
      '',
      '## How to actually walk a Nguyễn tomb',
      '',
      ...
      '',
      '## Getting there without the midday heat',
      '',
  ...
  },
  {
    ...
    tags: [
      ...
      { slug: 'ninh-binh', name: 'Ninh Binh' },
    ],
    content: [
      '## Day one: Tam Coc by rowboat',
      '',
      ...
      '',
      '## Mua Cave and the five hundred steps',
      '',
      ...
      '',
      '## Day two: Tràng An is the longer story',
      '',
      ...
      '',
      '## Bích Động pagoda, built into the rock itself',
      '',
  ...
  },
  {
    ...
    content: [
      '## Thirty-six streets, thirty-six guilds',
      '',
      ...
      '',
      '## How to actually cross the street',
      '',
      ...
      '',
      '## Where your feet actually get tired',
      '',
  ...
  },
  {
    ...
    tags: [
      { slug: 'nature', name: 'Nature' },
      { slug: 'lan-ha-bay', name: 'Lan Ha Bay' },
    ],
    content: [
      '## Why the same ten karsts show up in every photo',
      '',
      ...
      '',
      '## What an overnight actually buys you',
      '',
  ...
  },
  {
    ...
    content: [
      '## A dragon that breathes fire on weekends',
      '',
      ...
      '',
      '## Bún chả cá, not to be confused with cao lầu',
      '',
  ...
  },
  {
    ...
    tags: [{ slug: 'practical', name: 'Practical' }],
    content: [
      '## The north runs on its own calendar',
      '',
      ...
      '',
      '## The centre answers to typhoons, not seasons',
      '',
      ...
      '',
      '## The south stays warm and rarely commits to a downpour',
      '',
      ...
      '',
      '## If you can only pick one month',
      '',
